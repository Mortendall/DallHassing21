...
count_matrix_assembly <- function(file_type){
    count_file <- fs::dir_ls(here::here("data-raw/RNAseq/"),
                             regexp = file_type,
                             recurse = TRUE)
    count_matrix <- data.table::fread(count_file, data.table = F)
